...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @else
            <style>
                /* Minimal fallback styles so layout não quebre sem Vite */
                html,body{font-family:Figtree,ui-sans-serif,system-ui,sans-serif;margin:0;padding:0}
                .sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
            </style>
        @endif
    </head>
    <body class="font-sans antialiased bg-white dark:bg-black text-gray-700 dark:text-white/70">
        <div class="min-h-screen bg-gray-50 dark:bg-zinc-900">
            <div class="max-w-7xl mx-auto px-6 py-8">
                <nav class="flex items-center justify-between">
                    <a href="/" class="flex items-center gap-3">
                        <div class="h-10 w-10 rounded-full bg-[#0ea5a3] flex items-center justify-center text-white font-semibold">C</div>
                        <div class="text-lg font-semibold text-gray-900 dark:text-white">Collegio</div>
                    </a>

                    <div class="hidden md:flex items-center gap-6">
                        <a href="#sobre" class="text-sm hover:text-gray-900 dark:hover:text-white">Sobre</a>
                        <a href="#cursos" class="text-sm hover:text-gray-900 dark:hover:text-white">Cursos</a>
                        <a href="#contato" class="text-sm hover:text-gray-900 dark:hover:text-white">Contato</a>
                    </div>

                    @if (Route::has('login'))
                        <div class="flex items-center gap-3">
                            @auth
                                <a href="{{ url('/dashboard') }}" class="text-sm px-4 py-2 border rounded-md text-gray-800 dark:text-white">Dashboard</a>
                            @else
                                <a href="{{ route('login') }}" class="text-sm px-4 py-2">Entrar</a>

                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="text-sm px-4 py-2 bg-[#0ea5a3] text-white rounded-md shadow-sm">Criar conta</a>
                                @endif
                            @endauth
                        </div>
                    @endif
                ...
                <header class="mt-16 grid gap-8 lg:grid-cols-2 items-center">
                    <div>
                        <h1 class="text-4xl sm:text-5xl font-extrabold text-gray-900 dark:text-white leading-tight">Educação de excelência para o futuro do seu filho</h1>
                        <p class="mt-6 text-lg text-gray-600 dark:text-white/70">Ambiente acolhedor, metodologia ativa e foco no desenvolvimento integral — acadêmico, social e emocional.</p>

                        <div class="mt-8 flex gap-4">
                            <a href="#inscricoes" class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#0ea5a3] text-white rounded-md shadow hover:bg-[#08998f]">Inscreva-se</a>
                            <a href="#sobre" class="inline-flex items-center justify-center gap-2 px-6 py-3 border rounded-md text-gray-800 dark:text-white">Saiba mais</a>
                        </div>

                        <ul class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600 dark:text-white/70">
                            <li class="flex items-start gap-3"><span class="h-6 w-6 bg-[#0ea5a3] text-white rounded-full flex items-center justify-center text-xs">✓</span> Corpo docente qualificado</li>
                            <li class="flex items-start gap-3"><span class="h-6 w-6 bg-[#0ea5a3] text-white rounded-full flex items-center justify-center text-xs">✓</span> Infraestrutura moderna</li>
                            <li class="flex items-start gap-3"><span class="h-6 w-6 bg-[#0ea5a3] text-white rounded-full flex items-center justify-center text-xs">✓</span> Projetos interdisciplinares</li>
                            <li class="flex items-start gap-3"><span class="h-6 w-6 bg-[#0ea5a3] text-white rounded-full flex items-center justify-center text-xs">✓</span> Acompanhamento individual</li>
                        </ul>
                    </div>

                    <div class="order-first lg:order-last">
                        <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-lg overflow-hidden">
                            <img src="/images/school-hero.jpg" alt="Colégio" class="w-full h-64 object-cover object-center">
                        </div>
                    </div>
                </header>

                <section id="cursos" class="mt-16">
                    <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">Nossos diferenciais</h2>
                    <div class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        <div class="p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-sm">
                            <h3 class="font-semibold text-lg text-gray-900 dark:text-white">Ensino personalizado</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-white/70">Planos pedagógicos adaptados às necessidades de cada aluno.</p>
                        </div>

                        <div class="p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-sm">
                            <h3 class="font-semibold text-lg text-gray-900 dark:text-white">Laboratórios e oficinas</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-white/70">Estrutura para prática científica, tecnológica e artística.</p>
                        </div>

                        <div class="p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-sm">
                            <h3 class="font-semibold text-lg text-gray-900 dark:text-white">Programa socioemocional</h3>
                            <p class="mt-2 text-sm text-gray-600 dark:text-white/70">Apoio ao desenvolvimento de competências socioemocionais.</p>
                        </div>
                    </div>
                </section>

                <section id="contato" class="mt-16 bg-white dark:bg-zinc-800 p-6 rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Fale conosco</h2>
                    <p class="mt-2 text-sm text-gray-600 dark:text-white/70">Agende uma visita ou tire suas dúvidas — responderemos em breve.</p>

                    <form action="{{ route('contact.send') ?? '#' }}" method="POST" class="mt-4 grid gap-4 sm:grid-cols-2">
                        @csrf
                        <input name="name" placeholder="Nome" class="p-3 rounded-md border border-gray-200 bg-white dark:bg-zinc-900" />
                        <input name="email" placeholder="E-mail" class="p-3 rounded-md border border-gray-200 bg-white dark:bg-zinc-900" />
                        <textarea name="message" placeholder="Mensagem" class="p-3 rounded-md border border-gray-200 bg-white dark:bg-zinc-900 sm:col-span-2"></textarea>
                        <button type="submit" class="sm:col-span-2 inline-flex justify-center px-6 py-3 bg-[#0ea5a3] text-white rounded-md">Enviar mensagem</button>
                    </form>
                </section>

                <footer class="mt-16 py-8 text-center text-sm text-gray-600 dark:text-white/50">
                    &copy; {{ date('Y') }} Collegio — Todos os direitos reservados.
                </footer>
            </div>
        </div>
    </body>
